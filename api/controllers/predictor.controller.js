...
module.exports.addMockTest = (req, res, next) => {
    ...
    History.find({ userEmailID: req.body.userEmailID }, (err, document) => {
        if (!err && document.length == 1) {
            console.log(document[0].mockTests)
...
module.exports.predictColleges = async (req, res, next) => {
    ...
    Range.find({}, (err, ranges) => {
        if (!err) {
            ...
            res.send({ "DreamColleges": dreamColleges, "ReachColleges": reachColleges, "SafeColleges": safeColleges })
        }
                ...
                for (i in rows) {
                    ...
                    if (i > 0) {
                        ...
                        for (j in rows[i]) {
                            ...
                            if (j == 0) { range.rangeID = rows[i][j]; }


                            if (j == 1) { range.score = rows[i][j]; }


                            if (j == 2) { range.colleges = String(rows[i][j]).split(","); }
